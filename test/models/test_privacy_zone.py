...
Either we:
1) Mock the destructive methods, like:
> with patch.object(Picture, 'remove_exif_coordinates', return_value=None, autospec=True) as mock_method:
...
class TestPrivacyFilterOnImage:
    ...
    def test_privacy_filter_is_applied_if_close(self, image_path_close_to_ristafallet,
                                                privacy_zone_ristafallet):
        applied = PrivacyZone.apply_many_on_processed_picture(
            [privacy_zone_ristafallet],
            image_path_close_to_ristafallet
        )

        assert applied

    def test_privacy_filter_is_not_applied_if_far(self, image_path_far_from_ristafallet_waterfall,
                                                  privacy_zone_ristafallet):
        applied = PrivacyZone.apply_many_on_processed_picture(
            [privacy_zone_ristafallet],
            image_path_far_from_ristafallet_waterfall
        )
...
class TestPrivacyFilterOnTrack:
    ...
    def test_privacy_filter_is_applied_if_close(self, track_path_close_to_ristafallet, privacy_zone_ristafallet):
        applied = PrivacyZone.apply_many_on_processed_track(
            [privacy_zone_ristafallet],
            track_path_close_to_ristafallet
        )

        assert applied

    def test_privacy_filter_is_not_applied_if_far(self, track_path_far_from_ristafallet,
                                                  privacy_zone_ristafallet):
        applied = PrivacyZone.apply_many_on_processed_track(
            [privacy_zone_ristafallet],
            track_path_far_from_ristafallet
        )
